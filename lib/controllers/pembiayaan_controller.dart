...
import '../repository/pembiayaan_repository.dart' as repository;
  ...
  void simpanPengajuan() async{
    ...
    if (formPembiayaanTunai.currentState.validate()) {
      ...
      repository.pengajuanTunai(pengajuan).then((value) {
        if (value != null && value.slug != null) {
            
            Navigator.of(scaffoldKey.currentContext).pushReplacementNamed('/PembiayaanTunaiDetail', arguments: RouteArgument(param: value.toMap()));
        }else {
          ScaffoldMessenger.of(scaffoldKey?.currentContext).showSnackBar(SnackBar(
